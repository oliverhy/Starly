...
7. 也可以手动编辑文字后点击“发送文字”。
8. 切换到“Codex 任务”，可查看额度和最近任务；点击任务后可阅读最近消息、继续发送要求或停止运行中的任务。

### Codex 状态和额度说明

- “剩余额度”是 Codex 官方接口返回的当前使用窗口数据，由“100% − 已使用百分比”计算，不代表账户现金余额。
- 重置时间由 Codex 返回，手机按本地时间显示。
- “正在执行”表示当前 Starly 的 Codex App Server 会话确认该任务处于活动状态；其他 Codex 进程尚未载入的历史任务显示为“已保存”。
